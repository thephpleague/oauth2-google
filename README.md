...
[Google Sign In](https://developers.google.com/identity/sign-in/web/sign-in) wiill also need to be set up, which will provide you with the `{google-app-id}` and `{google-app-secret}` required (see [Usage](#usage) below).
